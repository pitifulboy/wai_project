import numpy as np
import pandas as pd
from get_data_from_waizao.cal_gainian import get_gainian_df
...
def bankuai_analysis(tradedata, bk_type):
    ...
    elif bk_type == '归属行业板块名称' or bk_type == '归属地域板块名称':
        # 透视分析
        df_povit = pd.pivot_table(tradedata, values=['成交额（元）', '涨跌幅度（%）', '股票代码', '总市值（元）', '流通市值（元）'], index=bk_type,
                                  aggfunc={'总市值（元）': np.sum, '流通市值（元）': np.sum, '成交额（元）': np.sum, '涨跌幅度（%）': np.mean,
                                           '股票代码': np.count_nonzero})
